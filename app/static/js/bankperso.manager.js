...
var $ShowProcessInfo = {
    ...
    show: function(x) {
        ...
        this.status = getattr(x, 'status', '');
        this.path = getattr(x, 'path', null);

        if (is_null(x) || is_empty(this.status)) {
            $NotificationDialog.open(keywords['No documents'], 340, height, keywords['Processing info']);
            return;
        }

        var info = this.status.match( /\[(.*?)\]/i );
    ...
    refresh: function(ob) {
        ...
        var action = '314';
        var params = {'command':'process-info'};

        this.init();

        $web_logging(action, function(x) { $ShowProcessInfo.show(x); }, params);
    },
};
