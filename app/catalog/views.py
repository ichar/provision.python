...
@catalog.route('/%s' % default_page, methods=['GET', 'POST'])
@catalog.route('/%s' % default_locator, methods=['GET', 'POST'])
#@login_required
def start():
    try:
        return index()
...
def index():
    debug, kw = init_response('Products Catalog Page')
    ...
    try:
        ...
        if IsTrace:
            print_to(errorlog, '--> catalog:%s %s [%s:%s] %s %s' % ( \
    ...
    return make_response(render_template('catalog/%s.html' % default_locator, debug=debug, **kw))

@catalog.after_request
...
@catalog.route('/%s/loader' % default_locator, methods = ['GET', 'POST'])
#@login_required
def loader():
    exchange_error = ''
    exchange_message = ''
