...
from ..utils import checkPaginationRange, getCurrency, getFloatMoney, getMaskedPAN, image_base64
...
default_locator = 'diamond'
default_template = 'provision-orders'

_PAGE_LINK = ('%sdiamond', '?sidebar=0',)
_IMAGE_SIZE_SCOPE = ((1300, 120, 240, 1.3), (1400, 120, 240, 1.3), (1600, 120, 240, 1.4), (1900, 140, 280, 1.4))
...
<body>
  ...
  <table>
  <tr><td class="info">Просим в течение 2-х рабочих дней согласовать исполнение производственного заказа.</td></tr>
  <tr><td class="caption">Производственный заказ:</td></tr>
...
<body>
  <div class="box">
  <h1 class="center">Производственный заказ</h1>
  <table>
  <tr><td class="info">Создан новый производственный заказ.</td></tr>
...
<body>
  ...
  <h1 class="center">Производственный заказ</h1>
  <table>
  <tr><td class="info">Производственный заказ удален.</td></tr>
  <tr><td class="caption">Заказ:</td></tr>
  <tr><td><dd class="order"><a target="_blank" href="webperso#args&_id=%(id)s">[%(id)05d]</a>&nbsp;%(Article)s</dd></td></tr>
  <tr><td><span class="info">%(Date)s</span></td></tr>
  <tr><td>
...
<body>
  ...
  <tr><td><span class="info">%(Date)s</span></td></tr>
  <tr><td>
    <div class="title">Запрос на согласование производственного заказа. %(Reviewer)s</div>
    <div class="message">%(Message)s</div>
  </td></tr>
  <tr><td><div class="line"><hr></div></td></tr>
  </table>
...
_config = {
    'provision-orders' : { 
        'columns' : ['TID', 'Article', 'Account', 'Qty', 'Category', 'Price', 'Currency', 'Total', 'Condition', 'Seller', 'Status', 'RD',],
        'noprice' : ['TID', 'Article', 'Account', 'Qty', 'Category', 'Seller', 'Status', 'RD',],
        'disable' : ['Author', 'Article', 'Subdivision', 'Seller',],
        'view'    : '[dbo].[WEB_Orders_vw]',
        'headers' : { 
            'TID'          : ('ID заявки',              'npp',          '', ''),
            'Article'      : ('НАИМЕНОВАНИЕ ИЗДЕЛИЯ',   'article',      '', ''),
            'Author'       : ('Автор документа',        '',             '', ''),
            'AuthorName'   : ('Заказчик',               '',             '', ''),
            'Qty'          : ('Кол-во, шт.',            'qty',          '', ''),
            'Category'     : ('Категория',              '',             '', ''),
            'Purpose'      : ('СПЕЦИФИКАЦИИ',           '',             '', ''),
        ...
        'status'  : ('Qty', 'Price',),
        'updated' : ('Author', 'Article', 'Qty', 'Purpose', 'Price', 'Currency', 'Account', 'URL', 'EditedBy', 'SubdivisionID', 'ConditionID', 'EquipmentID', 'SellerID', 'CategoryID',),
...
class PageDiamond(PageDefault):
    ...
    def confirmation_message(self):
        return 'Пожалуйста, ознакомьтесь с информацией о согласовании производственного заказа'
    ...
    def get_review_alarm_title(status, order_status):
        return status == 5 and 'Обоснование по производственному заказу' or 'Информация %s производственного заказа' % (
    ...
    def is_page_manager(self):
        return (self.is_owner or self.is_diamond_manager) and True or False
    @property
    ...
    def set_attrs(self, ob, attrs):
        setattr(ob, '_account', attrs.get('order_account'))

        return {
            'Login'         : (0, ob.login, 'Автор заявки',),
            'Subdivision'   : (1, ob._subdivision, 'ПОДРАЗДЕЛЕНИЕ',),
            'Article'       : [2, ob._article, 'Наименование изделия',], 
            ...
            'Seller'        : (8, ob._seller, 'ЗАКАЗЧИК',),
            'SellerTitle'   : (9, ob._seller_title, 'Реквизиты заказчика',),
            ...
            'Author'        : (15, ob._author, 'Менеджер',), 
            'Category'      : (16, ob._category, 'Категория',),
            'URL'           : (17, ob._url, 'Ссылка на товар на сайте Компании (URL)',),
            }

    def get_order_params(self, data):
        for row in data['data']:
            if row.get('Name') in ('Номер карты', 'PAN'):
                row['Value'] = getMaskedPAN(row['Value'], with_space=True)
        return data

    def make_current_order_info(self, order_id, info, **kw):
        """
            Изображения в контенте инфопанели
        """
        images = []

        if not info:
            info = {}
        if not info.get('stocklist'):
            info['stocklist'] = {}
        info['stocklist']['title'] = 'Класс продукции'

        # Get downloaded documents and check images only
        documents = [ob for ob in self._get_order_documents(order_id)['data'] if ob.get('image-type')]
        if not documents:
            return info

        min_width, max_width, scope = 120, 160, 1
        if self.params:
            w = int(self.params.get('max-width', 2100))
            for x in _IMAGE_SIZE_SCOPE:
                if w < x[0]:
                    min_width, max_width, scope = x[1], x[2], len(documents) == 1 and x[3] or 1
                    break
        size = ([int(min_width*scope), int(max_width*scope)], [120, 190])
        ...
        for n in range(0,2):
            if not documents:
                ...
                ob = documents.pop()
                uid = ob['UID']
                image_type = ob.get('image-type')
                if uid and image_type:
                    image = self.image_loader.download(uid, with_commit=False)
                    images.append(image_base64(None, image_type, size=size, image=image, optimize=True))
        if images:
            info['images'] = images
        return info

    def documents_action(self, order_id, info):
        info.append(self.make_current_order_info(order_id))
        return info
    ...
    def access_query(self, args):
        ...
        if aq and self.is_with_reviewers_template:
            aq = '(%s)' % ' or '.join(list(filter(None, [aq, "[dbo].CHECK_IsInReviewers_fn(TID, '%s')=1" % (
    ...
    def render(self, kw, back_where=None, uncheck_status=None):
        ...
        """
        self.point('diamond.render-start')
            ...
            if cursor:
                ...
                for n, row in enumerate(cursor):
                    ...
                    if not self._is_no_price:
                        total_sum += self._calc_euro(total, row['Currency'])

                    row['Total'] = getCurrency(total)
                    row['Tax'] = getCurrency(row['Tax'], driver=self.driver)
        ...
        self.point('diamond.render-finish')
