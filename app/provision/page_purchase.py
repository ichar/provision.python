...
from .page_default import PageDefault, Seller
...
default_locator = 'purchase'
default_template = 'provision-orders'

_PAGE_LINK = ('%spurchase', '?sidebar=0',)
...
<body>
  <div class="box">
  <h1 class="center">Контракт на закупку</h1>
  <table>
...
<body>
  ...
  <tr><td><span class="info">%(Date)s</span></td></tr>
  <tr><td>
    <div class="title">Запрос на согласование контракта на закупку. %(Reviewer)s</div>
    <div class="message">%(Message)s</div>
  </td></tr>
  <tr><td><div class="line"><hr></div></td></tr>
  </table>
...
class PagePurchase(PageDefault):

    def __init__(self, id, name, title, caption, **kw):
        if IsDeepDebug:
            print('PagePurchase init')
    ...
    def confirmation_message(self):
        return 'Пожалуйста, ознакомьтесь с информацией о согласовании коммерческого документа'
    ...
    def is_page_manager(self):
        return (self.is_owner or self.is_purchase_manager) and True or False
    @property
    def page_link(self):
        return _PAGE_LINK
    @property
    ...
    def get_review_alarm_title(status, order_status):
        return status == 5 and 'Обоснование по договору закупки' or 'Информация %s договора закупки' % (
    ...
    def system_config_emails_approval(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_APPROVAL or []
    @property
    def system_config_emails_create(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_CREATE or []
    @property
    def system_config_emails_remove(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_REMOVE or []
    @property
    def system_config_emails_review(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_REVIEW or []
    @property
    def system_config_emails_execute(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_EXECUTE or []
    @property
    def system_config_emails_payments(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_PAYMENTS or []
    @property
    def system_config_emails_audit(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_AUDIT or []
    @property
    def system_config_emails_decree(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_DECREE or []
    @property
    def system_config_emails_upload(self):
        return g.system_config.DEFAULT_EMAILS_PURCHASE_UPLOAD or []
    ...
    def ref_sellers(self):
        ...
        sellers += [(x['TID'], Seller.render_name(x['Name'], x['Type'])) for x in cursor if x['MD'] in (None, 0, self.model)]
        return sellers
    ...
    def render(self, kw, back_where=None, uncheck_status=None):
        ...
        """
        self.point('purchase.render-start')
        ...
        if self.engine != None:
            ...
            self.point('purchase.render-1.1')
            ...
            self.point('purchase.render-1.2')
            ...
            self.point('purchase.render-1.3')
            ...
            self.point('purchase.render-1.4')
        ...
        self.point('purchase.render-finish')
